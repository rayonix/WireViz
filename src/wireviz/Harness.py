...
class Harness:
    ...
    def _get_fiber_spec_string(self, cable: Cable) -> str:
        """Generate fiber specification string for display."""
        specs = []
        
        if cable.core_diameter and cable.cladding_diameter:
            specs.append(f"{int(cable.core_diameter)}/{int(cable.cladding_diameter)}μm")
        elif cable.core_diameter:
            specs.append(f"{int(cable.core_diameter)}μm core")
            
        if cable.fiber_type:
            if cable.fiber_type == "single-mode":
                specs.append("SM")
            elif cable.fiber_type == "multi-mode":
                specs.append("MM")
            else:
                specs.append(cable.fiber_type.upper())
        
        if cable.wavelength:
            if isinstance(cable.wavelength, list):
                wavelengths = "/".join(str(w) for w in cable.wavelength)
            else:
                wavelengths = str(cable.wavelength)
            specs.append(f"{wavelengths}nm")
        
        if cable.numerical_aperture:
            specs.append(f"NA {cable.numerical_aperture}")
